...
run_nipah_discrete <- function(param, max_time, time_step, initial_cond){
  ...
  E_b <- rep(NA, nsteps); E_b[1] = initial_cond['E_b']
  ...
  out <- data.frame(cbind(time, S_b, E_b, I_b, incidI_d,
                          S_d, S_a, I_d,incidI_b,
                          S_h, E_h, I_h, R_h, N_h, incidI_h)) 
...
# assign values to parameters
 ## For bats
  V_b = 0.003 #birth rate of bats
  mu_b = 0.0005 #death rate of bats
  beta_b = 0.0045 #transmission coefficient among bats
  beta_bd = 0.4 #transmission coefficient from bats to susceptible date palm trees
  lambda_b = 1/7 #1/latent period in bats
  ...
  R_da = 1  #relative preference of date palm tree over alternative plant 
                    # (equal preference: R_da = 1)
  V_d = 0  #birth rate of date palm trees
  V_a = 0  #birth rate of alternative plants
  mu_d = 0  #death rate of date palm trees
  mu_a = 0 #death rate of alternative plants
  theta_d = 1/60  #pot replacement rate 
  gamma_d = 0.4  #viral inactivation rate in date palm tree sap
  
  
  ## For humans
  beta_h = 0 #transmission coefficients among humans
  beta_dh = 0.4 #transmission coefficient from infectious date palm trees to humans
...
param = list(
  ...
  lambda_b = lambda_b, #1/latent period in bats
  ...
  R_da = R_da,  #relative preference of date palm tree over alternative plant 
                    # (equal preference: R_da = 1)
  V_d = V_d,  #birth rate of date palm trees
  V_a = V_a , #birth rate of alternative plants
  mu_d = mu_d , #death rate of date palm trees
  mu_a = mu_a,  #death rate of alternative plants
...
Tree = 100 * 1000000
time_step = 1 
initial_cond = c(S_b=11000, E_b=0, I_b=0,
                 S_d=Tree*param$P_d, S_a=Tree*param$P_a, I_d=0,
                 S_h=10000000, E_h=0, I_h=0, R_h=0)
...
lines(result$time, result$E_b, type="l", lwd=2, col="red")
...
# humans
plot(result$time, result$S_h, type="l", xlim = c(0, max_time), ylim = c(0, 100000000), lwd=2, col="blue", xlab = 'time', ylab = 'number', main = "humans")
...
legend('topright', legend = c('S', 'E', 'I','R'), col = c('blue', 'lightblue', 'red','green'), bty = 'n', pch = 15, ncol = 1)
